import torch
from UlterFastLaneDetection.utilities.backbone import resnet, mobilenet
import numpy as np
...
class parsingNet(torch.nn.Module):
    def __init__(self, size=(288, 800), pretrained=True, backbone='large', cls_dim=(37, 10, 4), use_aux=False):
        ...
        self.model = mobilenet(backbone, pretrain=pretrained)
        ...
        self.pool = torch.nn.Conv2d(960,8,1) if backbone in ['34','18'] else torch.nn.Conv2d(960,8,1)
        # 1/32,2048 channel
        # 288,800 -> 9,40,2048
        # (w+1) * sample_rows * 4
        # 37 * 10 * 4
    ...
    def forward(self, x):
        ...
        if self.use_aux:
            ...
            aux_seg = None
        #print("fea_1",fea.shape) #
        fea = self.pool(fea).view(-1, 1800)
        #print("fea_2",fea.shape)
        group_cls = self.cls(fea).view(-1, *self.cls_dim)
        #print('cls',group_cls.shape)
        if self.use_aux:
